...
public interface StudentGroupQuery extends StudentQuery {
    /** Returns student groups, where both groups and student within a group are ordered by name. */
    List<Group> getGroupsByName(Collection<Student> students);

    /** Returns student groups, where groups are ordered by name, and student within a group are ordered by id. */
    List<Group> getGroupsById(Collection<Student> students);

    /**
     * Returns name of the group containing maximum number of student.
    ...
    /**
     * Returns name of the group containing maximum number of student with distinct first names.
     * If there are more than one largest group, the one with smallest name is returned.
     */
